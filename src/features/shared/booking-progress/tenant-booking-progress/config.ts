...
export default function useTenantBookingProgress(
  ...
  // CANCELLED_BOOKING
  // REJECTED_BOOKING
  // const status = "COMPLETED_BOOKING";

  // For testing: you can swap status here
  ...
  const paymentState: RenderStateViewProps["state"] = {
    ...
    message: isRejected
      ...
      ? "..."
      : isPaymentApproval
      ? "Waiting for payment approval from owner."
      : isCompleted
      ? "Owner has accepted payment."
      : "",
...
function getInitialColorConfig(status: string, colors: typeof colorPresets) {
  ...
  if (["CONFIRMED_REQUEST"].includes(status)) return colors.processing;
