...
export default function RoomsCheckoutScreen() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Failed to create Booking Request: " + (error?.error || "Unknown error")
      );
    }
  };
    ...
    >
      ...
      <VStack style={{ flex: 1, padding: Spacing.md }}>
        ...
        <Box>
          ...
          <Box>
            <Button onPress={() => setShowCheckIn(true)}>
              <Text style={[s.textColor, s.button_text_style]}>Pick Date</Text>
            </Button>
            {showCheckIn && (
              <DateTimePicker
                value={checkIn}
        ...
        <Box>
          <Box>
            <Text style={[{ marginTop: 20 }, s.textColor]}>
        ...
        <Box style={[{ marginTop: "auto" }]}>
          <Button onPress={() => setShowModal(true)}>
            <Text style={[{}, s.textColor]}>Book</Text>
          </Button>
        </Box>
      </VStack>
      <DecisionModal
